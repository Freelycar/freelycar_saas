...
public class SafeConnection {
    ...
    /**
     * MUST MATCH THE SERVER KEY, 必须和API服务器KEY一致 MUST KEEP THIS KEY SAFE，这个KEY请勿泄露
     */
...
//        logger.info(jsonData);
    ...
    public static <T> T decodeFromString(String s, Class<T> classOfT) throws IOException {
        Gson gson = new Gson();
        DataWithDigest dataWithDigest = gson.fromJson(s, SafeConnection.DataWithDigest.class);
        return getOrignalObject(dataWithDigest, classOfT);
